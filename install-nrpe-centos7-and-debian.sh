...
echo "If Debian, manually add exception for tcp port 5666 in iptables"
if [[ ! -z $YUM_CMD ]]; then
    firewall-cmd --permanent --zone=public --add-rich-rule='rule family="ipv4" source address="192.168.200.29/22" port protocol="udp" port="5666$
    firewall-cmd --reload
